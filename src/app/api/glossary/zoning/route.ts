    ...
    const insertedOrUpdated = await sql`
      WITH data AS (
        SELECT * FROM jsonb_to_recordset(${JSON.stringify(payload)}::jsonb) AS (
          ...
          code_token_kind land_v2.code_token_kind_t,
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const res = await sql`DELETE FROM land_v2.glossary_zoning WHERE jurisdiction_display = ${jurisdiction_display} AND local_code_raw = ${code} RETURNING 1`
    return NextResponse.json({ deleted: (res as any[]).length ?? 0 })
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
